...
public class sample1_remote2 {
	...
	public static void main(String[] args) throws Exception {
		...
		Callable<Integer> callable2 = new Callable<Integer>() {
			...
			public Integer call() throws Exception {
				WebDriver driver = new RemoteWebDriver(
						new URL("http://10.141.212.23:4444/wd/hub"), 
                        DesiredCapabilities.chrome());
		...
		Callable<Integer> callable3 = new Callable<Integer>() {
			...
			public Integer call() throws Exception {
				WebDriver driver = new RemoteWebDriver(
						new URL("http://10.141.212.24:4444/wd/hub"), 
                        DesiredCapabilities.chrome());
		...
		for (int i = 0; i < 3; i++) {
			FutureTask<Integer> futureTask = new FutureTask<Integer>(callable3);
			futureTasks.add(futureTask);
			executorService.submit(futureTask);
		}
