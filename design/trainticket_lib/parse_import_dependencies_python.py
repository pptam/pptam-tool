...
def extract_service_dependencies(root_dir, subfolders):
    ...
    for subfolder in subfolders:
        ...
        logging.debug(f"Analyzing folder {full_path}...")
        ...
        if os.path.isdir(target_path):
            for file_name in os.listdir(target_path):
                if file_name.endswith('.jar'):
                    ...
                    logging.debug(f"Including jar {jar_path}...")
...
def run_analysis(config_file):
    ...
    if "hard_coded_results" in config:
        return [config["hard_coded_results"]]

    subfolders_to_traverse = config.get("subfolders_to_traverse", [])
    target_package = config.get("target_package", "")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Analyze import dependencies or load hardcoded results.")
