...
from createTestConfigurationInterfaceHandler import executionAndMeasurementProduceOutputHandler
...
def create_test(configuration_file_path, test_plan_file_path, configuration_entries_to_overwrite):
    ...
    with open(test_plan_file_path, "r") as f:
        ...
        for test in testPlan:
            ...
            ## Handle specific test configuration ############################################################
            ##################################################################################################
            executionAndMeasurementProduceOutputHandler(path_to_temp, path_to_output, path_to_drivers, test_id, executionAndMeasurementOpt, executionAndMeasurementConf)
            ##################################################################################################  
