...
def deploy(current_configuration, design_path, output, test_id):
    if current_configuration["docker_deploy"]=="1":
        ...
        print(command_check_folder, design_path)
        result = run_external_application(command_check_folder, False)
        if result != 0:
            source_path = os.path.join(design_path, "..", "..", "design", "hotelreservation", "*")
            command_copy = f'scp -r "{design_path}" {target_machine}:"{target_folder}"'
            # run_external_application(command_copy)
        
        deployment_descriptor = os.path.join(design_path, "docker-compose.yml")
        target_deployment_descriptor = os.path.join(target_folder, "hotelreservation", "DeathStarBench", "hotelReservation","docker-compose.yml")
        command_copy_compose = f'scp "{deployment_descriptor}" {target_machine}:{target_deployment_descriptor}'
