...
def run(current_configuration, output, test_id):
    ...
    run_external_applicaton(
        f'locust --locustfile {driver} --host {host} --users {load} --spawn-rate {spawn_rate} --run-time {run_time}s --headless --only-summary --reset-stats --csv {csv_prefix} --csv-full-history --logfile "{log_file}" --loglevel DEBUG > {out_file} 2>&1', False)
