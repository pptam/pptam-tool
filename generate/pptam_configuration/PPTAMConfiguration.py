...
class PPTAMConfiguration():
    ...
    def readConfiguration(self, configurationFilePath):
        ...
        if(configurationFilePath.endswith('txt')):
            ...
            json.dump(self.parseConfiguration.getJSON_Dictionary(),
    ...
    def generateTests(self, testExecutorOrigin, fabanDriverDestination, driversDestination):
        ...
        # Copy FABAN files
        commonMethods.copyFromFolder2Folder(
            fabanDriverEcsaOrigin, driversDestinationTmp)
        ...
        commonMethods.replaceValueInFile("${FABAN_IP}", pptamConfigurationData.getFabanIP(
        ), driversDestinationTmp+"/build.properties")
        # Configure the Test ID - build.properties
        commonMethods.replaceValueInFile(
            "${TEST_NAME}", testID, driversDestinationTmp+"/build.properties")
        # Configure JAVA_HOME_FABAN - run.xml
        commonMethods.replaceValueInFile(
            "${JAVA_HOME_FABAN}", pptamConfigurationData.getJavaHomeFaban(), driversDestinationTmp+"/build.xml")

        # Remove bak files
        commonMethods.removeFileRelativePath(
            driversDestinationTmp+"/build.properties.bak")
        commonMethods.removeFileRelativePath(
            driversDestinationTmp+"/build.xml.bak")
        ...
        commonMethods.replaceValueInFile(
            "${TEST_NAME}", testID, pathTmpDeployRun)
        # Configure FABAN_IP - run.xml
        commonMethods.replaceValueInFile(
            "${FABAN_IP}", pptamConfigurationData.getFabanIP(), pathTmpDeployRun)
        # Configure NUM_USERS - run.xml
        commonMethods.replaceValueInFile(
            "${NUM_USERS}", fabanConfigurationData.getNumberOfUsers(), pathTmpDeployRun)
        # Configure FABAN_OUTPUT_DIR - run.xml
        commonMethods.replaceValueInFile(
            "${FABAN_OUTPUT_DIR}", pptamConfigurationData.getfabanOutputDir(), pathTmpDeployRun)
        # Configure SUT_IP - run.xml
        commonMethods.replaceValueInFile(
            "${SUT_IP}", pptamConfigurationData.getSutIP(), pathTmpDeployRun)
        # Configure SUT_PORT - run.xml
        commonMethods.replaceValueInFile(
            "${SUT_PORT}", pptamConfigurationData.getSutPort(), pathTmpDeployRun)
        ...
        # Remove bak file.
        commonMethods.removeFileRelativePath(
            driversDestinationTmp+"/deploy/run.xml.bak")
    # rm ./drivers/tmp/deploy/run.xml.bak

        # Configure testID - WebDriver.java
        commonMethods.replaceValueInFile(
            "${TEST_NAME}", testID, driversDestinationTmp+"/src/ecsa/driver/WebDriver.java")

        # Remove bak file
        commonMethods.removeFileRelativePath(
            driversDestinationTmp+"/src/ecsa/driver/WebDriver.java.bak")
        ...
        dockerFileOrigin = fabanTemplatesOrigin + \
            "/deployment_descriptor/template/docker-compose.yml"
        ...
        commonMethods.replaceValueInFile("${SUT_HOSTNAME}", pptamConfigurationData.getSutHostname(
        ), dockerFolderTarget+"/docker-compose.yml")
        # Configure CARTS_REPLICAS - docker-compose.yml
        commonMethods.replaceValueInFile("${CARTS_REPLICAS}", dockerConfigurationData.getNumOfReplicas(
        ), dockerFolderTarget+"/docker-compose.yml")
        # Configure CARTS_CPUS_LIMITS - docker-compose.yml
        commonMethods.replaceValueInFile("${CARTS_CPUS_LIMITS}", dockerConfigurationData.getCpuLimit(
        ), dockerFolderTarget+"/docker-compose.yml")
        # Configure CARTS_CPUS_RESERVATIONS - docker-compose.yml
        commonMethods.replaceValueInFile("${CARTS_CPUS_RESERVATIONS}", dockerConfigurationData.getCpuReservation(
        ), dockerFolderTarget+"/docker-compose.yml")
        # Configure CARTS_RAM_LIMITS - docker-compose.yml
        commonMethods.replaceValueInFile("${CARTS_RAM_LIMITS}", dockerConfigurationData.getRamLimit(
        ), dockerFolderTarget+"/docker-compose.yml")
        # Configure CARTS_RAM_RESERVATIONS - docker-compose.yml
        commonMethods.replaceValueInFile("${CARTS_RAM_RESERVATIONS}", dockerConfigurationData.getRamReservation(
        ), dockerFolderTarget+"/docker-compose.yml")

        commonMethods.removeFileRelativePath(
            dockerFolderTarget+"/docker-compose.yml.bak")
        ...
        commonMethods.copyFromFolder2Folder(
            driversDestinationTmp, driversDestination+"/"+testID)
        commonMethods.deleteFilesFromFolder(driversDestinationTmp)
        ...
        commonMethods.copyFileRel(driversDestination+"/"+testID+"/build/" +
                                  testID+".jar", testExecutorOrigin+"/to_execute/"+testID)
        # cp ./drivers/$TEST_ID/build/$TEST_ID.jar ./to_execute/$TEST_ID
        commonMethods.copyFileRel(driversDestination+"/"+testID +
                                  "/config/run.xml", testExecutorOrigin+"/to_execute/"+testID)
        # cp ./drivers/$TEST_ID/config/run.xml ./to_execute/$TEST_ID
        commonMethods.copyFileRel(driversDestination+"/"+testID +
                                  "/deploy/docker-compose.yml", testExecutorOrigin+"/to_execute/"+testID)
    ...
    def executeTests(self, rootDirectory):
        ...
        FABAN_MASTER = "http://"+pptamConfigurationData.getFabanIP()+":" + \
            pptamConfigurationData.getSutPort()+"/"
        ...
        testFolder = rootDirectory+"/run/to_execute/"
        ...
        for f in os.scandir(testFolder):
            ...
            if f.is_dir():
                ...
                terminalCmd = "java -jar "+FABAN_CLIENT+" "+FABAN_MASTER+" deploy "+test_name + \
                    " "+driver+" "+driver_conf + \
                    " | (read RUN_ID ; echo $RUN_ID > "+RUN_ID_FILE+")"
                os.system(terminalCmd)
                ...
                while ((STATUS != "COMPLETED") and (STATUS != "COMPLETED")):
                    ...
                    terminalCmd = "java -jar ./faban/benchflow-faban-client/target/benchflow-faban-client.jar"++ \
                        " status $RUN_ID | (read STATUS ; echo $STATUS > " + \
                        STATUS_FILE+")"
                ...
                os.mkdir(rootDirectory+"/run/executed/"+testID)
                os.mkdir(rootDirectory+"/run/executed/"+testID+"/faban")

                terminalCmd = "java -jar ./faban/benchflow-faban-client/target/benchflow-faban-client.jar " + \
                    FABAN_MASTER+" info "+RUN_ID+" > executed/"+testID+"/faban/runInfo.txt"
                os.system(terminalCmd)
...
def generate_test(NUM_USERS, CARTS_REPLICAS, CARTS_CPUS_LIMITS, CARTS_CPUS_RESERVATIONS, CARTS_RAM_LIMITS, CARTS_RAM_RESERVATIONS):
    ...
    myCmd = os.popen(
        'cat /dev/urandom | env LC_CTYPE=C tr -dc \'a-zA-Z0-9\' | fold -w 32 | head -n 1').read()
